...
class IKresult {
 public:
  ...
  solvers::SolutionResult& analytical_ik_status() {
    return analytical_ik_status_;
  }

  const solvers::SolutionResult& analytical_ik_status() const {
    return analytical_ik_status_;
  }

  solvers::SolutionResult& global_ik_status() { return global_ik_status_; }

  const solvers::SolutionResult& global_ik_status() const {
    return global_ik_status_;
  }
  ...
  std::vector<Eigen::Matrix<double, 6, 1>>& q_analytical_ik() {
    return q_analytical_ik_;
  }

  const std::vector<Eigen::Matrix<double, 6, 1>>& q_analytical_ik() const {
    return q_analytical_ik_;
  }

  Eigen::Matrix<double, 6, 1>& q_global_ik() { return q_global_ik_; }

  const Eigen::Matrix<double, 6, 1>& q_global_ik() const {
    return q_global_ik_;
  }
  ...
  const Eigen::Matrix<double, 6, 1>& q_nl_ik_resolve() const {
    return q_nl_ik_resolve_;
  }
...
class DUT {
 public:
  ...
  DUT(const Eigen::Quaterniond& ee_orient,
    ...
    global_ik_.AddBoundingBoxConstraint(Eigen::Vector3d(0, 0, -1),
                                        Eigen::Vector3d(0, 0, -1), R_02.col(1));
    global_ik_.AddBoundingBoxConstraint(0, 0, R_02(2, 2));
    const auto p_02 = global_ik_.body_position(2);
    global_ik_.AddBoundingBoxConstraint(
        Eigen::Vector2d::Zero(), Eigen::Vector2d::Zero(), p_02.head<2>());
  ...
  void SolveAnalyticalIK(const Eigen::Vector3d& link6_pos,
                         IKresult* ik_result) {
    ...
    ik_result->q_analytical_ik() =
        analytical_ik_.inverse_kinematics(link6_pose);
    if (ik_result->q_analytical_ik().size() > 0) {
      ik_result->analytical_ik_status() =
  ...
  void SolveGlobalIK(const Eigen::Vector3d& link6_pos, IKresult* ik_result) {
    ...
    // solvers::SolutionResult global_ik_status =
    // mosek_solver.Solve(global_ik_);
  ...
  void SolveNonlinearIK(
      const Eigen::Vector3d& link6_pos, IKresult* ik_result,
  ...
  void SolveIK(const Eigen::Vector3d& link6_pos, std::fstream* output_file) {
    ...
    if (ik_result.analytical_ik_status() ==
            solvers::SolutionResult::kSolutionFound &&
    ...
    while (getline(file, line)) {
      ...
      if (line == "orientation (quaternion):") {
        ...
        Eigen::Quaterniond quat(
            std::atof(quat_str[0].c_str()), std::atof(quat_str[1].c_str()),
            std::atof(quat_str[2].c_str()), std::atof(quat_str[3].c_str()));
      ...
      if (analytical_ik_status_str[0] == "analytical_ik_status:") {
        ik_result.analytical_ik_status() = static_cast<solvers::SolutionResult>(
            std::atoi(analytical_ik_status_str[1].c_str()));
      } else {
        throw std::runtime_error("oops");
      }
      ...
      if (line == "q_analytical:") {
        while (getline(file, line)) {
          ...
          if (q_analytical_ik_str[0] == "nonlinear_ik_status:") {
            ik_result.nl_ik_status() =
                std::atoi(q_analytical_ik_str[1].c_str());
            break;
          } else {
      ...
      if (global_ik_status_str[0] == "global_ik_status:") {
        ik_result.global_ik_status() = static_cast<solvers::SolutionResult>(
            std::atoi(global_ik_status_str[1].c_str()));
      } else {
        throw std::runtime_error("oops");
      }
      ...
      if (nl_ik_resolve_status_str[0] == "nonlinear_ik_resolve_status:") {
        ik_result.nl_ik_resolve_status() =
            std::atoi(nl_ik_resolve_status_str[1].c_str());
      } else {
        throw std::runtime_error("oops");
      }
      ...
      if (line == "q_nonlinear_ik_resolve:") {
        ...
        for (int i = 0; i < 6; ++i) {
          ik_result.q_nl_ik_resolve()(i) =
              std::atof(q_nl_ik_resolve_str[i].c_str());
        }
      } else {
        throw std::runtime_error("oops");
      }
...
void DebugOutputFile(int argc, char* argv[]) {
  ...
  for (auto& ik_result : ik_results) {
    ...
    solvers::SolutionResult::kSolutionFound
      ...
      EXPECT_TRUE((ik_result.q_analytical_ik()[0].array() >=
    robot->joint_limit_min.array()).all());
      EXPECT_TRUE((ik_result.q_analytical_ik()[0].array() <=
    robot->joint_limit_max.array()).all());
      cache.initialize(ik_result.q_analytical_ik()[0]);
      robot->doKinematics(cache);
      std::array<Eigen::Isometry3d, 6> link_pose;
      for (int i = 0; i < 6; ++i) {
        link_pose[i] = robot->CalcBodyPoseInWorldFrame(cache,
    *(robot->FindBody("link_" + std::to_string(i + 1))));
      }
    ...
    solvers::SolutionResult::kSolutionFound) {
      ...
      if (pos_error <= 0.04) {
        dut.SolveGlobalIK(ik_result.ee_pose().translation(), &ik_result);
        dut.SolveNonlinearIK(ik_result.ee_pose().translation(), &ik_result,
    ik_result.q_global_ik(), true);
        ik_result.printToFile(&output_file1);
      }
    }*/
    /*if (ik_result.global_ik_status() ==
    solvers::SolutionResult::kInvalidInput) {
      dut.SolveGlobalIK(ik_result.ee_pose().translation(), &ik_result);
      ik_result.printToFile(&output_file1);
    }*/
    /*if ((ik_result.global_ik_status() ==
...
void AnalyzeNonlinearIKresult(const std::vector<IKresult>& ik_results) {
  ...
  int resolve_feasible_count =
      0;  // IK fails initially, but succeeds with global
          // IK result as the seed.
  int initial_feasible_count =
      0;  // IK succeeds initially, but fails with global
          // IK result as the seed.
  ...
  std::cout << "Both initial IK and resolve IK succeed: " << both_feasible_count
            << std::endl;
  std::cout << "Neither initial IK nor resolve IK succeed: "
            << both_infeasible_count << std::endl;
  std::cout << "Initial IK fails, but resolve IK succeed: "
            << resolve_feasible_count << std::endl;
  std::cout << "Initial IK succeeds, but resolve IK fails: "
            << initial_feasible_count << std::endl;
}
...
void AnalyzeOutputFile(int argc, char* argv[]) {
  ...
  for (const auto& ik_result : ik_results) {
    ...
    } else if ((ik_result.global_ik_status() ==
                    solvers::SolutionResult::kInfeasible_Or_Unbounded ||
                ik_result.global_ik_status() ==
                    solvers::SolutionResult::kInfeasibleConstraints) &&
               ik_result.analytical_ik_status() ==
  ...
  for (int i = 0; i < static_cast<int>(both_feasible.size()); ++i) {
    ...
    q_global_ik_at_joint_limits[i] =
        ((both_feasible[i].q_global_ik() - dut.robot()->joint_limit_min)
             .array() < 1E-2)
            .any() ||
        ((both_feasible[i].q_global_ik() - dut.robot()->joint_limit_max)
             .array() > -1E-2)
            .any();

    dut.robot()->doKinematics(cache);
    Eigen::Isometry3d ee_pose_global_ik = dut.robot()->CalcBodyPoseInWorldFrame(
        cache, dut.robot()->get_body(dut.ee_idx()));
    global_ik_ee_error(0, i) = (ee_pose_global_ik.translation() -
                                both_feasible[i].ee_pose().translation())
                                   .norm();
    global_ik_ee_error(1, i) =
        Eigen::AngleAxisd(ee_pose_global_ik.linear().transpose() *
                          both_feasible[i].ee_pose().linear())
            .angle();
    if (q_global_ik_at_joint_limits[i]) {
      global_ik_ee_error_at_joint_limits.conservativeResize(
          Eigen::NoChange, global_ik_ee_error_at_joint_limits.cols() + 1);
      global_ik_ee_error_at_joint_limits.col(
          global_ik_ee_error_at_joint_limits.cols() - 1) =
          global_ik_ee_error.col(i);
    } else {
      global_ik_ee_error_not_at_joint_limits.conservativeResize(
          Eigen::NoChange, global_ik_ee_error_not_at_joint_limits.cols() + 1);
      global_ik_ee_error_not_at_joint_limits.col(
          global_ik_ee_error_not_at_joint_limits.cols() - 1) =
          global_ik_ee_error.col(i);
    }
  }
  ...
  CallMatlab("hold", "on");
  auto h_pose_error_at_joint_limits =
      CallMatlab(1, "plot", global_ik_ee_error_at_joint_limits.row(0) * 100.0,
                 global_ik_ee_error_at_joint_limits.row(1) / M_PI * 180.0);
  CallMatlab("set", h_pose_error_at_joint_limits[0], "LineStyle", "none",
             "Marker", "o", "Color", Eigen::Vector3d(1, 0, 0));
  CallMatlab("set", h_pose_error_at_joint_limits[0], "DisplayName",
             "Joint limits active");

  auto h_pose_error_not_at_joint_limits = CallMatlab(
      1, "plot", global_ik_ee_error_not_at_joint_limits.row(0) * 100.0,
      global_ik_ee_error_not_at_joint_limits.row(1) / M_PI * 180.0);
  CallMatlab("set", h_pose_error_not_at_joint_limits[0], "LineStyle", "none",
             "Marker", "x", "Color", Eigen::Vector3d(0, 0, 1));
  CallMatlab("set", h_pose_error_not_at_joint_limits[0], "DisplayName",
             "Joint limits inactive");
