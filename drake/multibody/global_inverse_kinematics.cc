...
GlobalInverseKinematics::GlobalInverseKinematics(
  ...
  R_WB_binary_.resize(num_bodies);
  R_WB_phi_.resize(num_bodies);
        ...
        switch (joint->get_num_velocities()) {
          ...
          case 1: {
            ...
            if (dynamic_cast<const RevoluteJoint*>(joint) != nullptr) {
              ...
              // the program.
              std::tie(R_WB_binary_[body_idx], R_WB_phi_[body_idx]) =
          ...
          if (dynamic_cast<const RevoluteJoint*>(joint) != nullptr) {
            ...
            if (joint_bound < M_PI) {
              ...
              if (constraint_types.find(ConstraintType::kSecondOrderCone) !=
                ...
                // 1+2*cos(α) ≤ trace(R(k, (a+b)/2)ᵀ * R_PFᵀ * R_WPᵀ * R_WC) ≤ 3
