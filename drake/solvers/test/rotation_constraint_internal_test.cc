...
/**
 * Cut each axis at [-1, -0.5, 0, 0.5, 1], use logarithmic sos2 constraint to
 * encode in which box the vector lies, and then check if the integer-valued
...
class TestCalcBoxBinaryExpressionInOrthant : public ::testing::TestWithParam<std::tuple<int, int, int, int, int>> {
 ...
 protected:
  ...
  int zi_;
  int orthant_;
  std::array<VectorXDecisionVariable, 3> lambda_;
  std::array<VectorXDecisionVariable, 3> B_;
};

TEST_P(TestCalcBoxBinaryExpressionInOrthant, TestBoxBinaryExpression) {
  CheckSolution();
}
