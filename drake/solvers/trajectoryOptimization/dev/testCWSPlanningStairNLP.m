...
lfoot_pos0(1) = lfoot_pos0(1)-0.003;
rfoot_pos0 = rfoot_pos_star;
rfoot_pos0(1) = rfoot_pos0(1)+0.003;
cnstr = {WorldPositionConstraint(robot,l_foot,[0;0;0],lfoot_pos0(1:3),lfoot_pos0(1:3));...
  WorldQuatConstraint(robot,l_foot,lfoot_pos0(4:7),0);...
  WorldPositionConstraint(robot,r_foot,[0;0;0],rfoot_pos0(1:3),rfoot_pos0(1:3));...
  WorldQuatConstraint(robot,r_foot,rfoot_pos0(4:7),0)};
[q0,info] = robot.inverseKin(qstar,qstar,cnstr{:});
kinsol0 = robot.doKinematics(q0,vstar);
...
lfoot_contact_pos1 = lfoot_contact_pos0+repmat([box_size(1)+0.025;0;box_size(3)],1,4);
rfoot_contact_pos1 = rfoot_contact_pos0+repmat([box_size(1)+0.025;0;box_size(3)],1,4);

lfoot_contact_wrench0 = struct('active_knot',1:lfoot_takeoff_idx,'cw',lfoot_cw,'contact_pos',lfoot_contact_pos0);
lfoot_contact_wrench1 = struct('active_knot',lfoot_land_idx:nT,'cw',lfoot_cw,'contact_pos',lfoot_contact_pos1);
rfoot_contact_wrench0 = struct('active_knot',1:rfoot_takeoff_idx,'cw',rfoot_cw,'contact_pos',rfoot_contact_pos0);
...
% lfoot_off_ground
lfoot_offground_cnstr1 = WorldPositionConstraint(robot,l_foot,l_foot_contact_pts,[nan(2,4);0.1*ones(1,4)],repmat([box_tops(1,2)-box_size(1)/2-0.03;box_size(2)/2;nan(1,1)],1,4));
fccdfkp = fccdfkp.addConstraint(lfoot_offground_cnstr1,{lfoot_takeoff_idx+1});
lfoot_offground_cnstr2 = WorldPositionConstraint(robot,l_foot,l_foot_contact_pts,[nan(2,4);(box_tops(3,2)+0.02)*ones(1,4)],[repmat([box_tops(1,2)-box_size(1)/2;nan(2,1)],1,2) repmat([box_tops(1,2)-box_size(1)/2;box_size(2)/2;nan(1,1)],1,2)]);
fccdfkp = fccdfkp.addConstraint(lfoot_offground_cnstr2,{lfoot_takeoff_idx+2});
lfoot_offground_cnstr3 = WorldPositionConstraint(robot,l_foot,l_foot_contact_pts,[nan(2,4);(box_tops(3,2)+0.03)*ones(1,4)],repmat([box_tops(1,2)+box_size(1)/2-0.03;box_size(2)/2;nan(1,1)],1,4));
fccdfkp = fccdfkp.addConstraint(lfoot_offground_cnstr3,{lfoot_takeoff_idx+3});

% rfoot_off_ground
rfoot_offground_cnstr1 = WorldPositionConstraint(robot,r_foot,r_foot_contact_pts,[nan(1,4);-box_size(2)/2*ones(1,4);0.1*ones(1,4)],repmat([box_tops(1,2)-box_size(1)/2-0.03;nan(2,1)],1,4));
fccdfkp = fccdfkp.addConstraint(rfoot_offground_cnstr1,{rfoot_takeoff_idx+1});
rfoot_offground_cnstr2 = WorldPositionConstraint(robot,r_foot,r_foot_contact_pts,[nan(1,4);-box_size(2)/2*ones(1,4);(box_tops(3,2)+0.02)*ones(1,4)],[repmat([box_tops(1,2)-box_size(1)/2;nan(2,1)],1,2) repmat([box_tops(1,2)-box_size(1)/2;nan(2,1)],1,2)]);
fccdfkp = fccdfkp.addConstraint(rfoot_offground_cnstr2,{rfoot_takeoff_idx+2});
rfoot_offground_cnstr3 = WorldPositionConstraint(robot,r_foot,r_foot_contact_pts,[nan(1,4);-box_size(2)/2*ones(1,4);(box_tops(3,2)+0.03)*ones(1,4)],repmat([box_tops(1,2)+box_size(1)/2-0.03;nan(2,1)],1,4));
...
% fix initial posture
fccdfkp = fccdfkp.addConstraint(ConstantConstraint(qstar([floating_base_idx;back;l_arm;r_arm])),fccdfkp.q_inds([floating_base_idx;back;l_arm;r_arm],1));

% Do not bend the knee too much
fccdfkp = fccdfkp.addConstraint(BoundingBoxConstraint(-inf(nT,1),1.4*ones(nT,1)),fccdfkp.q_inds(l_leg_kny,:));
fccdfkp = fccdfkp.addConstraint(BoundingBoxConstraint(-inf(nT,1),1.4*ones(nT,1)),fccdfkp.q_inds(r_leg_kny,:));
...
[cws_margin_sol,l0,l1,l2,l3,l4,V,solver_sol,info] = prog_lagrangian.findCWSmargin(sol.cws_margin/2,sol.friction_cones,sol.grasp_pos,sol.grasp_wrench_vert,disturbance_pos,sol.momentum_dot,sol.com);
keyboard
end