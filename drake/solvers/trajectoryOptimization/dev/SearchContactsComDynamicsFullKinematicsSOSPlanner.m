classdef SearchContactsComDynamicsFullKinematicsSOSPlanner < ContactWrenchSetDynamicsFullKineamticsPlanner
  ...
  methods
    function obj = SearchContactsComDynamicsFullKinematicsSOSPlanner(robot,N,tf_range,Q_comddot,Qv,Q,cws_margin_cost,q_nom,contact_wrench_struct,Qw,options)
      ...
      if(~isfield(options,'sos_file_name'))
        options.sos_file_name = 'sosCoefficientFun';
      end
      if(~isfield(options,'sos_param_file'))
        options.sos_param_file = 'sosParamterFile.mat';
      end
      ...
      obj.sos_file_name = options.sos_file_name;
      obj.sos_param_file = options.sos_param_file;
    end
    ...
    function obj = addSOScondition(obj)
      if(~obj.checkSOSparamterFile())
        ...
        for i = 1:obj.N
          ...
          [V_indet,V_power,V_coeff] = decomp(V(i),decision_var);
          decision_sym = sym('x',[length(decision_var),1]);
          assumeAlso(decision_sym, 'real');
          sparse_pattern = zeros(length(V_coeff),length(decision_var));
          for j = 1:length(V_coeff)
            coeff_var = decomp(V_coeff(j));
            match_ij = match(decision_var,coeff_var);
            sparse_pattern(j,match_ij) = 1;
            V_coeff_fun(j) = msspoly2sym(coeff_var,decision_sym(full(match_ij)),V_coeff(j));
            dV_coeff = diff(V_coeff(j),decision_var(full(match_ij)));
            dV_coeff_fun(j) = msspoly2sym(coeff_var,decision_sym(full(match_ij)),dV_coeff);
          end
          matlabFunction(V_coeff_fun,'File',sprintf('%s%d',obj.sos_file_name,i),'Var',decision_sym);
          matlabFunction(dV_coeff_fun,'File',sprintf('d%s%d',obj.sos_file_name,i),'Var',decision_sym);
        end
      end
    end
    
    function flag = checkSOSparameterFile(obj)
      if(~exist(obj.sos_param_file,'file'))
        flag = false;
        return;
      end
      param = load(obj.sos_param_file);
      flag = valuecheck(param.robot_mass,obj.robot_mass);
      flag = valuecheck(param.gravity,obj.gravity) & flag;
      flag = valuecheck(param.N,obj.N) & flag;
      flag = valuecheck(param.Qw,obj.Qw) & flag;
      flag = valuecheck(param.num_fc_edges,obj.num_fc_edges) & flag;
      flag = valuecheck(param.use_lin_fc,obj.use_lin_fc) & flag;
      for i = 1:obj.N
        flag = valuecheck(param.fc_axis{i},obj.fc_axis{i}) & flag;
        flag = valuecheck(param.fc_mu{i},obj.fc_mu{i}) & flag;
        flag = valuecheck(param.num_grasp_wrench_vert{i},obj.num_grasp_wrench_vert{i}) & flag;
        for j = 1:obj.num_grasp_pts(i)
          flag = valuecheck(param.grasp_wrench_vert{i}{j},obj.grasp_wrench_vert{i}{j}) & flag;
        end
      end
    end
    
    function writeSOSparamterFile(obj)
      if(exist(obj.sos_param_file,'file'))
        delete(obj.sos_param_file);
      end
      robot_mass = obj.robot_mass;
      save(obj.sos_param_file,'robot_mass','-append');
      gravity = obj.gravity;
      save(obj.sos_param_file,'gravity','-append');
      N = obj.N;
      save(obj.sos_param_file,'N','-append');
      Qw = obj.Qw;
      save(obj.sos_param_file,'Qw','-append');
      num_fc_edges = obj.num_fc_edges;
      save(
    end
  end
end