function worldPositionTest
  ...
  if visualize
    lcmgl = LCMGLClient('worldPositionTest');
  ...
  ub = lb;
  lfoot_on_ground_cnstr = DrakeFunctionConstraint(lb,ub,dist_to_ground_fcn(lfoot_pts_fcn));

  lb = zeros(rfoot_pts_fcn.n_pts,1);
  ub = lb;
  rfoot_on_ground_cnstr = DrakeFunctionConstraint(lb,ub,dist_to_ground_fcn(rfoot_pts_fcn));
