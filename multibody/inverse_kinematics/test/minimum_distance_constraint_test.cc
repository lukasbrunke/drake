...
class NFreeSpheresModel {
 public:
  explicit NFreeSpheresModel(int num_spheres) : num_spheres_{num_spheres} {
    ...
    for (int i = 0; i < num_spheres; ++i) {
      ...
      sphere_id_.push_back(plant_->RegisterCollisionGeometry(
          body, X_BS, geometry::Sphere(radius_),
  ...
  const std::vector<geometry::GeometryId>& sphere_id() const { return sphere_id_; }
 ...
 private:
  ...
  std::vector<geometry::GeometryId> sphere_id_;
...
GTEST_TEST(ThreeSpheresTest, DistanceCrossInfluence) {
  // Make sure that the constraint value is continuous and differentiable when
  // the distance of one pair crosses d_influence.
  NFreeSpheresModel<double> three_spheres(3);
  // Ignore the collision between sphere 1 and sphere 2.
  three_spheres.scene_graph()
      .collision_filter_manager(
          &(three_spheres.scene_graph().GetMyMutableContextFromRoot(
              &(three_spheres.get_mutable_diagram_context()))))
      .Apply(geometry::CollisionFilterDeclaration().ExcludeWithin(
          geometry::GeometrySet{
              {three_spheres.sphere_id()[1], three_spheres.sphere_id()[2]}}));
  const double d_min = 0.01;
  const double d_influence = 0.06;
  // Indices into the q vector for each sphere's position.
  Eigen::Index kSpheres[] = {4, 11, 18};
  Eigen::VectorXd q_above_influence =
      three_spheres.plant().GetPositions(three_spheres.plant_context());
  // Position for sphere 0.
  q_above_influence.segment<3>(kSpheres[0]) << 0, 0, 0;
  // Position for sphere 1.
  q_above_influence.segment<3>(kSpheres[1]) << 0, 0,
      d_influence + 2 * three_spheres.radius() + 1E-5;
  // Position for sphere 2.
  q_above_influence.segment<3>(kSpheres[2]) << 0, 0,
      -d_influence - 2 * three_spheres.radius() + 1E-5;

  Eigen::VectorXd q_below_influence = q_above_influence;
  q_below_influence.segment<3>(kSpheres[1]) << 0, 0,
      d_influence + 2 * three_spheres.radius() - 1E-5;
  for (MinimumDistancePenaltyFunction penalty_function :
       {QuadraticallySmoothedHingeLoss, ExponentiallySmoothedHingeLoss}) {
    MinimumDistanceConstraint dut(&(three_spheres.plant()), d_min,
                                  &(three_spheres.get_mutable_plant_context()),
                                  penalty_function, d_influence - d_min);
    Eigen::VectorXd y_val_above_influence;
    dut.Eval(q_above_influence, &y_val_above_influence);
    Eigen::VectorXd y_val_below_influence;
    dut.Eval(q_below_influence, &y_val_below_influence);
    std::cout << y_val_above_influence << "\n" << y_val_below_influence << "\n";
  }
}
}  // namespace multibody
}  // namespace drake
