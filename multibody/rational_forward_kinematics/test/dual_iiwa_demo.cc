...
void SetDiffuse(const MultibodyPlant<double>& plant,
  ...
  if (frame_id.has_value()) {
    for (const auto& geometry_id : inspector.GetGeometries(
      ...
      const auto old_rgba = props->GetProperty<geometry::Rgba>("phong", "diffuse");
      ...
      std::cout << rgba_r_val << " " << rgba_g_val << " " << rgba_b_val << " " << rgba_a_val << "\n";
      geometry::IllustrationProperties new_props =
          geometry::MakePhongIllustrationProperties(Eigen::Vector4d(rgba_r_val, rgba_g_val, rgba_b_val, rgba_a_val));
...
class DualIiwaDiagram {
 public:
  ...
  // @param num_dual_iiwa Set this to != 1 only if you visualize multiple postures. Set this to 1 one you search for Cspace polytopes.
  DualIiwaDiagram(bool end_effector_only, int num_dual_iiwa=1)
    ...
    for (int dual_iiwa_count = 0; dual_iiwa_count < num_dual_iiwa; ++dual_iiwa_count) {
    const auto left_iiwa = parser.AddModelFromFile(iiwa_file_path, fmt::format("left_iiwa{}", dual_iiwa_count));
    left_iiwas_.push_back(left_iiwa);

    const math::RigidTransformd X_WL(Eigen::Vector3d(0, -0.3, 0));
    plant_->WeldFrames(plant_->world_frame(),
                       plant_->GetFrameByName("iiwa_link_0", left_iiwa), X_WL);
    const auto right_iiwa = parser.AddModelFromFile(iiwa_file_path, fmt::format("right_iiwa{}", dual_iiwa_count));
    ...
    for (int dual_iiwa_count = 1; dual_iiwa_count < num_dual_iiwa;
      ...
      for (const auto& body_index :
        ...
        SetDiffuse(*plant_, scene_graph_, body_index, std::nullopt, diffuse(0), diffuse(1), diffuse(2), diffuse(3));
      }
      for (const auto& body_index :
  ...
  void ColorIiwa(const ModelInstanceIndex model_instance, const std::string& highlight_geo_name) {
    for (const auto& body_index : plant_->GetBodyIndices(model_instance)) {
      ...
      for (const auto& geo_id : scene_graph_->model_inspector().GetGeometries(frame_id, geometry::Role::kIllustration)) {
...
void SearchCspacePolytope(const std::string& write_file, const std::optional<std::string>& read_file=std::nullopt, bool do_bisection=true) { 
  ...
  dut.CspacePolytopeBilinearAlternation(
      q_star, filtered_collision_pairs, C_bilinear_start,
      d_bilinear_start, bilinear_alternation_option,
      solver_options, q0, std::nullopt, &cspace_free_region_solution,
...
void VisualizePostures(const std::string& read_file) {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = i + 1; j < 2; ++j) {
      ...
      A << Eigen::MatrixXd::Identity(nt, nt), -Eigen::MatrixXd::Identity(nt, nt);
  ...
  for (int i = 1; i < num_postures - 1; ++i) {
    t_interpolate.col(i) = t_interpolate.col(0) + (t_sol.col(1) - t_sol.col(0)) / (num_postures-1) * i;
  }
  ...
  for (int i = 0; i < num_postures; ++i) {
    iiwa_diagram.plant().SetPositions(
        &plant_context, iiwa_diagram.left_iiwas()[i], q_interpolate.col(i).head(nt/2));
    iiwa_diagram.plant().SetPositions(
        &plant_context, iiwa_diagram.right_iiwas()[i], q_interpolate.col(i).tail(nt/2));
  }
...
void VisualizeOnePosture(const std::string& read_file) {
  ...
  for (int sample_count = 0; sample_count < num_samples; sample_count++) {
    ...
    prog.AddQuadraticErrorCost(Eigen::MatrixXd::Identity(nq, nq), t_sample, t);
    const auto result = solvers::Solve(prog);
    DRAKE_DEMAND(result.is_success());
    Eigen::VectorXd t_proj = result.GetSolution(t);
    Eigen::VectorXd q_proj =
        2 * (t_proj.array().atan().matrix());
    plant.SetPositions(&plant_context, q_proj);
    double distance = CalcMinDistance(
        plant, iiwa_diagram.scene_graph().model_inspector(), plant_context, {});
    if (distance < min_distance) {
      q_sol = q_proj;
      t_sol = t_proj;
      std::cout << "sample_count: " << sample_count << " distance: " << distance << "\n";
    ...
    t_sol << 0.344939,  0.299676, -0.264628,  0.148839,  0.185361,  0.174295,  0.518743, 0.0345981,  0.406462,  0.489963, 0.0976667,  0.171192;
  }
  ...
  Eigen::Vector3d a_E;
  double b_E;
  CalcPlane(separating_plane_vars, t_sol, SeparatingPlaneOrder::kAffine, &a_E, &b_E);
  // In the expressed frame, the plane is
  // a_E.dot(p_EV) + b_E = 0
  // In the world frame, the plane is a_E.T * R_EW * p_WV + a_E.dot(p_EW) + b_E = 0.
  ...
  Eigen::Vector3d p_WL;
  plant.CalcPointsPositions(plant_context, plant.get_body(left_link).body_frame(), Eigen::Vector3d::Zero(), plant.world_frame(), &p_WL);
  const Eigen::Vector3d p_WL_project = p_WL + (-b_W - a_W.dot(p_WL))/(a_W.dot(a_W)) * a_W;
