...
static void BenchmarkPolynomialEvaluatePartial(
  ...
  const auto monomial_basis =
      internal::ComputeMonomialBasis<Eigen::Dynamic>(x_set, 3);
...
static void BenchmarkPolynomialEvaluatePartialAlternative(
    benchmark::State& state) {  // NOLINT
  // Exactly the same as BenchmarkPolynomialEvaluatePartial, just don't call EvaluatePartial.
  Eigen::Matrix<symbolic::Variable, 12, 1> x;
  for (int i = 0; i < 12; ++i) {
    x(i) = symbolic::Variable("x" + std::to_string(i));
  }
  ...
  env.insert(S_upper, Eigen::VectorXd::Ones(S_upper.rows()));
  for (auto _ : state) {
    monomial_basis.dot(S_val * monomial_basis);
  }
}

BENCHMARK(BenchmarkPolynomialEvaluatePartial);
BENCHMARK(BenchmarkPolynomialEvaluatePartialAlternative);
}  // namespace
}  // namespace symbolic
}  // namespace drake
