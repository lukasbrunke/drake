...
void Search(const std::optional<std::string>& load_V_init) {
  ...
  const std::vector<int> p_degrees{{4, 4, 4, 4, 4}};
  ...
  if (load_V_init.has_value()) {
    ...
    const RowVector3<symbolic::Polynomial> dVdq = V_init.Jacobian(x.head<3>());
    for (int i = 0; i < 2; ++i) {
  ...
  while (iter_count < max_iters) {
    ...
    Vector2<symbolic::Polynomial> l_sol;
    {
      ...
      AddControlLyapunovConstraint(&prog, x, z1_poly, z2_poly, lambda0, V_sol,
                                   rho, l, qdot, deriv_eps, p,
                                   state_constraints);
      solvers::SolverOptions solver_options;
      solver_options.SetOption(solvers::CommonSolverOption::kPrintToConsole, 1);
      RemoveTinyCoeff(&prog, 1E-9);
