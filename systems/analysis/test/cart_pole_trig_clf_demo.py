...
def construct_diagram():
    ...
    return diagram, cart_pole, scene_graph


def SwingUpTrajectoryOptimization():
    diagram, cart_pole, scene_graph = construct_diagram()
...
def FindClfInit(params: analysis.CartPoleParams, V_degree, x):
    ...
    derivative_ceq_lagrangian_degrees = [4]
...
def SearchWithSlackA(params: analysis.CartPoleParams, x, u_max, deriv_eps):
    ...
    search_options.lagrangian_step_backoff_scale = 0.05
    search_options.lyap_step_backoff_scale = 0.05
    ...
    load_clf = True
    if load_clf:
        with open("/home/hongkaidai/sos_clf_cbf_data/cart_pole/cartpole_trig_clf20.pickle", "rb") as input_file:
...
def SearchWTrigDynamics(params, x, u_max, deriv_eps):
    ...
    x_star = analysis.ToCartpoleTrigState(
        0.04*state_swingup[:, -18] + 0.96 * state_swingup[:, -17])
