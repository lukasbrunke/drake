classdef ContactWrenchConstraint < RigidBodyConstraint
  ...
  methods
    ...
    function [tau,dtau] = centroidalTorque(obj,t,kinsol,F)
      % Return the torque about the robot Center of Mass generated by the contact.
      % The input argument is the same as in torque function
      % @retval tau     -- The total torque at the CoM
      % @retval dtau    -- The gradient of tau w.r.t q and F
      if(obj.isTimeValid(t))
        A = obj.force(t);
        [com,Jcom] = obj.robot.getCOM(kinsol);
        [tau1,dtau1] = obj.torque(t,kinsol,F);
        force = A*F(:);
        tau = tau1-cross(com,force);
        dtau_tmp = dcross(com,force);
        dtau = dtau1-[dtau_tmp(:,1:3)*Jcom dtau_tmp(:,4:6)*A];
      else
        tau = [];
        dtau = [];
      end
  ...
  methods(Access = protected)
    function flag = checkForceSize(obj,F)
      flag = isnumeric(F) && numel(F) == obj.F_size(1)*obj.F_size(2) && size(F,2) == 1;
    end
  end
