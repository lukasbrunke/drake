classdef CoMWholeBodyKinPlanning < NonlinearProgramWKinsol
  % solve IK
%   min_q sum_i
  ...
  methods
    function obj = CoMWholeBodyKinPlanning(robot,t,q_nom_traj,fix_initial_state,x0,varargin)
      ...
      if(~isa(q_nom_traj,'Trajectory'))
        error('Drake:CoMWholeBodyKinPlanning:q_nom_traj should be a trajectory');
      end
      ...
      for i = 1:num_rbcnstr
          ...
          for j = t_start:obj.nT
            if(varargin{i}.isTimeValid(obj.t_knot(j)) && varargin{i}.active)
              if(~isempty(obj.qsc_weight_idx{j}))
                error('Drake:CoMWholeBodyKinPlanning:currently only support at most one QuasiStaticConstraint at an individual time');
              end
              cnstr = varargin{i}.generateConstraint(obj.t_knot(j));
        ...
        elseif(isa(varargin{i},'ContactWrenchConstraint'))
          ...
          error('Drake:CoMWholeBodyKinPlanning:Unsupported RigidBodyConstraint');
        end
      end
      obj.g = 9.8;
      m = obj.robot.getMass();
