 ...
 public:
  ...
  static double ReconstructJointAngleForRevoluteJoint(
    ...
    return GlobalInverseKinematics::ReconstructJointAngleForRevoluteJoint(
        R_WP, R_WB, R_PF, a_F, p_WB, p_WC, p_BC, beta, angle_lower,
        angle_upper);
  }
};
...
void CheckReconstructJointAngleForRevoluteJoint(
  ...
  auto Cost = [&](double theta) {
    const Eigen::Matrix3d R_FB =
        Eigen::AngleAxisd(theta, a_F).toRotationMatrix();
    const Eigen::Matrix3d R_err = R_WB - R_WP * R_PF * R_FB;
    const Eigen::Matrix3Xd pos_err =
        p_WB * Eigen::RowVectorXd::Ones(1, p_WC.cols()) +
        R_WP * R_PF * R_FB * p_BC - p_WC;
...
GTEST_TEST(GlobalInverseKinematics, ReconstructJointAngleForRevoluteJoint) {
  const Eigen::Matrix3d R_WP =
      Eigen::AngleAxisd(0.1 * M_PI, Eigen::Vector3d(0.2, 0.5, 1.2).normalized())
          .toRotationMatrix();
  const Eigen::Vector3d a_F = Eigen::Vector3d(0.2, -3.1, -.5).normalized();
  const Eigen::Matrix3d R_PF =
      Eigen::AngleAxisd(0.5 * M_PI,
                        Eigen::Vector3d(0.2, -0.5, 1.2).normalized())
          .toRotationMatrix();
  Eigen::Matrix3d R_WB =
      R_WP * R_PF * Eigen::AngleAxisd(-0.2 * M_PI, a_F).toRotationMatrix();
  ...
  CheckReconstructJointAngleForRevoluteJoint(R_WP, R_WB, R_PF, a_F, p_WB, p_WC,
                                             p_BC, 0, -0.4 * M_PI, 0.2 * M_PI);
  CheckReconstructJointAngleForRevoluteJoint(R_WP, R_WB, R_PF, a_F, p_WB, p_WC,
                                             p_BC, 0, -0.4 * M_PI + 2 * M_PI,
                                             0.2 * M_PI + 2 * M_PI);

  // The optimal is at lower bound.
  CheckReconstructJointAngleForRevoluteJoint(R_WP, R_WB, R_PF, a_F, p_WB, p_WC,
                                             p_BC, 0, 0 * M_PI, 0.2 * M_PI);
  // The optimal is at upper bound.
  CheckReconstructJointAngleForRevoluteJoint(R_WP, R_WB, R_PF, a_F, p_WB, p_WC,
                                             p_BC, 0, -1.2 * M_PI, -0.4 * M_PI);

  // beta is non-zero. The optimal is where the gradient vanishes.
  CheckReconstructJointAngleForRevoluteJoint(
      R_WP, R_WB, R_PF, a_F, p_WB, p_WC, p_BC, 0.5, -0.4 * M_PI, 0.2 * M_PI);
  CheckReconstructJointAngleForRevoluteJoint(R_WP, R_WB, R_PF, a_F, p_WB,
  p_WC,
                                             p_BC, 5, -0.4 * M_PI, 0.2 * M_PI);

  // The optimal is at lower bound.
  CheckReconstructJointAngleForRevoluteJoint(R_WP, R_WB, R_PF, a_F, p_WB, p_WC,
                                             p_BC, 2, 0.2 * M_PI, 0.6 * M_PI);
  // The optimal is at upper bound.
  CheckReconstructJointAngleForRevoluteJoint(R_WP, R_WB, R_PF, a_F, p_WB, p_WC,
                                             p_BC, 2, -1.2 * M_PI, -0.5 *
  M_PI);
